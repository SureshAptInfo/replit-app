...
import WhatsAppMessage from "@/components/leads/whatsapp-message";
...
export default function LeadDetail() {
  ...
  // Find the assigned user from team members
  const assignedUser = teamMembers.find((member) => member.id === lead.assignedTo);
  ...
  const updateLeadStatusMutation = useMutation({
    mutationFn: async (newStatus: string) => {
      ...
      return { updatedLead, oldStatus, newStatus };
    },
    onSuccess: (data) => {
      ...
      // Safely access the status from returned data
      const newStatus = data?.newStatus || data?.updatedLead?.status || "updated";
      const oldStatusValue = data?.oldStatus || "previous";
      ...
      console.log(`Lead status changed: ${oldStatusValue} -> ${newStatus}`);
  ...
  const handleSetFollowUp = () => {
    ...
    createTaskMutation.mutate({
      ...
      createdBy: user?.id || 0,
      priority: "medium",
      description: `Follow-up call with ${lead.name}`
    });
  };
  ...
  useEffect(() => {
    if (isEditContactModalOpen && lead) {
      ...
      <div className="fixed inset-0 bg-neutral-100 flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b bg-white flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-heading font-semibold">Lead Details</h1>
          </div>
          
          <div className="flex items-center space-x-2">
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="border-primary border-2 text-primary font-semibold">
                  Change Status
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="flex items-start mb-6">
            <div className="w-12 h-12 rounded-full bg-primary text-white flex items-center justify-center mr-4 text-lg">
              <span>{getInitials(lead.name)}</span>
            </div>
            <div className="flex-1">
              <div className="flex items-center flex-wrap gap-2">
                <h3 className="font-heading font-semibold text-xl">{lead.name}</h3>
              </div>
              <div className="mt-1 text-sm text-neutral-500">
                <p>Created: {new Date(lead.createdAt).toLocaleString()}</p>
                <p>Source: {lead.source || "Manual Entry"}</p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button 
                      variant="outline" 
                      className="border-primary border-2 text-primary font-semibold h-9"
                    >
                      Change Status
                    </Button>
                  </PopoverTrigger>
                  ...
                  <div className="flex items-center">
                    <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center mr-2">
                      <span className="text-xs font-medium text-primary">
                        {assignedUser ? getInitials(assignedUser.username) : "NA"}
                      </span>
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-neutral-500">
                        {lead.lastActivity ? `Last activity: ${new Date(lead.lastActivity).toLocaleDateString()}` : ''}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {/* Add Note, Set Follow-up & WhatsApp Messaging */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Add Note */}
            <div className="bg-white rounded-lg shadow p-4">
