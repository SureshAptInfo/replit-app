...
export default function Messages() {
  ...
  // Fetch messages (activities) for selected contact with auto-refresh
  const { data: messages = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const messages = activities
        ...
        .map((activity: any) => ({
          ...
          isIncoming: activity.direction === 'incoming' || activity.direction === 'inbound',
          status: 'read',
    ...
    refetchInterval: 3000 // Refresh every 3 seconds to catch new WhatsApp messages
  });
  ...
  const isWhatsAppWindowOpen = selectedContact && messages.length > 0 ? (() => {
    // Find the most recent incoming WhatsApp message
    const whatsappMessages = messages.filter((msg: Message) => 
      msg.channel === 'whatsapp' && 
      msg.isIncoming === true
    );
    
    if (whatsappMessages.length === 0) {
      console.log('No incoming WhatsApp messages found');
      return false;
    }
    
    // Sort by timestamp to get the most recent
    const sortedMessages = whatsappMessages.sort((a: Message, b: Message) => 
      new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()
    );
    
    const lastIncomingWhatsApp = sortedMessages[0];
    const timeDiff = Date.now() - new Date(lastIncomingWhatsApp.timestamp).getTime();
    const hoursAgo = timeDiff / (1000 * 60 * 60);
    
    console.log(`Last incoming WhatsApp: ${lastIncomingWhatsApp.timestamp}, Hours ago: ${hoursAgo.toFixed(1)}`);
